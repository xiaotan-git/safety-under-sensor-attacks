...
def main():
    ...
    x_tr_mtx_de, u_seq_mtx_de = simulation(True,True,True,True)

    x_tr_mtx_nom, u_seq_mtx_nom = simulation(False,False,False,False)
    x_tr_lst.append(x_tr_mtx_nom)
    u_lst.append(u_seq_mtx_nom)

    # brute-force SSR-based safety filter is on
    x_tr_mtx_bf, u_seq_mtx_bf = simulation(True,True,False,False)
    x_tr_lst.append(x_tr_mtx_bf)
    u_lst.append(u_seq_mtx_bf)

    # # # decomposition SSR-based safety filter is on
    x_tr_mtx_de_ssr, u_seq_mtx_de_ssr = simulation(True,False,True,False)
    x_tr_lst.append(x_tr_mtx_de_ssr)
    u_lst.append(u_seq_mtx_de_ssr)

    # # decomposition M-based safety filter is on
    x_tr_mtx_de, u_seq_mtx_de = simulation(True,False,False,True)
    ...
    linestyles = ['-', '--', '-.', ':']
    colors = plt.cm.get_cmap('tab10', len(x_tr_lst))  # Automatically get a colormap
    ...
    plt.grid(True)
    plt.legend()
    plt_box(x_min=-10, x_max=10, y_min=-10, y_max=10, color='black', linewidth=4)
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig("figures/closed_system_trajectories.pdf")
